...
export const DashboardPage: React.FC<DashboardPageProps> = ({ onCreateTender }) => {
  const { role } = useRole();
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const rawTenders = await api.getAllTenders();
        const mapped = (rawTenders || []).map((t: any) => ({
          id: t.tenderId,
          ...
          bidAmount: 'N/A' // Not tracking contractor bid globally yet
        }));
        setTenders(mapped);
      } catch(err) {
        console.error(err);
  ...
  const myTendersCount = tenders.length.toString();
  const pendingCount = tenders.filter(t => t.status === 'open').length.toString();
  // We approximate total bids until a dedicated chaincode indexer is available
  const totalBidsCount = (tenders.length * 3).toString(); 
  const wonCount = tenders.filter(t => t.status === 'awarded').length.toString();

  const stats = [
    { id: 'tenders', label: role === 'issuer' ? 'My Tenders' : 'Active Tenders', value: myTendersCount, icon: FileText, color: 'text-blue-600', bg: 'bg-blue-50' },
    { id: 'my-bids', label: role === 'bidder' ? 'My Bids' : 'Total Bids', value: role === 'bidder' ? '2' : totalBidsCount, icon: TrendingUp, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { id: 'pending', label: 'Pending Approval', value: pendingCount, icon: Clock, color: 'text-amber-600', bg: 'bg-amber-50' },
    { id: 'won', label: role === 'bidder' ? 'Bids Won' : 'Network Uptime', value: role === 'bidder' ? wonCount : '99.9%', icon: CheckCircle2, color: 'text-slate-600', bg: 'bg-slate-50' },
  ];
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          {role === 'issuer' && (
            <Button onClick={onCreateTender} className="bg-blue-600 hover:bg-blue-700">
