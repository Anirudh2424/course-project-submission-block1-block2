...
export const LoginPage: React.FC<LoginPageProps> = ({ onLogin, onPublicClick }) => {
  ...
  const handleAuth = async () => {
    if (!selectedRole && authMode === 'signup') {
      toast.error("Please select an access type for your new account.");
      return;
    }
    if (!userId) {
    ...
    try {
        if (authMode === 'signup') {
            await api.signup({ userId, password, role: selectedRole, walletId });
            toast.success("Account created successfully! Please sign in.");
            setAuthMode('signin');
            setPassword('');
        } else {
            if (!selectedRole) {
               toast.error("Please select your access type.");
               return;
            }
            const res = await api.login({ userId, password, requestedRole: selectedRole });
