...
import { ContractManagement } from './components/pages/ContractManagement';
import { Toaster, toast } from 'sonner@2.0.3';

type AppView = 'landing' | 'login' | 'dashboard' | 'create-tender' | 'tender-list' | 'submit-bid' | 'result' | 'transparency' | 'audit' | 'settings' | 'manage-contract';
...
export default function App() {
  ...
  const renderDashboardContent = () => {
    switch (view) {
      case 'dashboard':
        return <DashboardPage onCreateTender={() => setView('create-tender')} />;
      case 'create-tender':
        return <CreateTender onPublish={handlePublishSuccess} />;
      case 'tender-list':
        return <TenderList onViewDetails={(id) => { setSelectedTenderId(id); setView('submit-bid'); }} onManageContract={(id) => { setSelectedTenderId(id); setView('manage-contract'); }} />;
      case 'submit-bid':
      ...
      case 'manage-contract':
        return <ContractManagement tenderId={selectedTenderId || '1'} onBack={() => setView('dashboard')} />;
      case 'result':
